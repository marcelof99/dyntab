...
const DrillDownTable = ({ config }) => {
  ...
  // Tracks which *individual regions* are expanded (vs showing a single collapsed region column)
  const [expandedRegions, setExpandedRegions] = useState(() => {
     // Initialize with all collapsible regions expanded by default
     const initialExpanded = new Set();
     get(config, 'columnDimensions[0].columns', []).forEach(region => {
       if (region.canCollapse) { // Only add collapsible regions initially
         initialExpanded.add(region.id);
       }
     });
     return initialExpanded;
   });
  // State for focused drilldown (e.g., showing only 'americas' data for 'Q1' months)
  const [activeBreakdown, setActiveBreakdown] = useState(null); // { parentRowId: string, columnId: string } | null
  ...
  const regionColumns = mainColumnDimension ? mainColumnDimension.columns : []; // e.g., Americas, EMEA, APAC configs
  // Flatten the lowest level columns (countries) for iteration and width calculation
  const countryColumns = useMemo(() => {
    return regionColumns.flatMap(region => region.columns ? region.columns.map(country => ({ ...country, regionId: region.id })) : []);
  }, [regionColumns]);
  ...
  const columnTotals = useMemo(() => {
    if (!countryColumns || countryColumns.length === 0) return {};
    const totals = countryColumns.reduce((acc, country) => {
      acc[country.id] = 0;
      return acc;
    }, {});
    ...
    topLevelRowData.forEach(item => {
      regionColumns.forEach(region => {
         if (region.columns) {
            region.columns.forEach(country => {
                const path = `values.${region.id}.${country.id}`;
    ...
    return totals;
  }, [topLevelRowData, regionColumns, countryColumns]);

  // Calculate the grand total across all lowest level columns (countries)
  const grandTotal = useMemo(() => {
     if (!countryColumns || countryColumns.length === 0) return 0;
     // Sum the calculated country totals
     return countryColumns.reduce((sum, country) => sum + get(columnTotals, country.id, 0), 0);
  }, [columnTotals, countryColumns]);

  // Calculate the total width of the lowest level data columns (countries)
  const totalDataColumnsWidth = useMemo(() => {
    if (!countryColumns || countryColumns.length === 0) return 200; // Default fallback
    return countryColumns.reduce((sum, country) => sum + (country.width || 90), 0); // Default width if not specified
  }, [countryColumns]);
  ...
  // --- Recursive function to calculate aggregated values ---
  const calculateRowAggregatedValue = useMemo(() => {
    const memo = new Map();

    const calculate = (item, targetRegionId = null, targetCountryId = null) => {
      const memoKey = `${item.id}-${targetRegionId}-${targetCountryId}`;
      if (memo.has(memoKey)) {
      ...
      if (item.children && item.children.length > 0) {
        // If item has children, sum their values recursively
        item.children.forEach(child => {
          totalValue += calculate(child, targetRegionId, targetCountryId);
        });
      } else {
        // Base case: No children, get value from this item's 'values' object
        if (targetRegionId && targetCountryId) {
          // Specific country value needed
          totalValue = get(item, `values.${targetRegionId}.${targetCountryId}`, 0);
        } else if (targetRegionId) {
          // Specific region total needed (sum its countries)
          const region = regionColumns.find(r => r.id === targetRegionId);
          if (region && region.columns) {
             region.columns.forEach(country => {
                totalValue += get(item, `values.${targetRegionId}.${country.id}`, 0);
             });
          }
        } else {
          // Grand total for this item needed (sum all regions/countries)
           regionColumns.forEach(region => {
              if (region.columns) {
                 region.columns.forEach(country => {
                    totalValue += get(item, `values.${region.id}.${country.id}`, 0);
                 });
              }
           });
        }
      ...
      memo.set(memoKey, totalValue);
      return totalValue;
    };
    return calculate; // Return the inner function
  }, [regionColumns]); // Dependency: regionColumns for summing within region/all
  ...
  const visibleColumns = useMemo(() => {
    ...
    // Otherwise, build list based on expanded regions
    const columns = [];
    regionColumns.forEach(region => {
        const isExpanded = expandedRegions.has(region.id);
        if (isExpanded && region.columns) {
            // Add all country columns for this expanded region
            region.columns.forEach(country => {
                columns.push({ 
                    ...country, 
                    regionId: region.id, 
        ...
        } else if (region.canCollapse) {
            // Add a single placeholder for the collapsed region
             columns.push({ 
                 id: region.id, 
                 title: region.title, 
                 width: region.collapsedWidth || 100, 
                 isCollapsedRegion: true,
                 regionId: region.id // Keep regionId for consistency
             });
        } else if (region.columns) {
            // Region cannot collapse, always show countries
             region.columns.forEach(country => {
                columns.push({ ...country, regionId: region.id, isCountry: true });
            });
        }
        // Handle regions without countries? (Edge case - add placeholder?)
        // else { columns.push({ id: region.id, title: region.title, width: 100, isPlaceholder: true }); }
    });
    return columns;
  }, [allColumnsCollapsed, expandedRegions, regionColumns, mainColumnDimension]);
  ...
  const toggleRow = (rowId) => {
    ...
    if (expandedRows.has(rowId)) {
      ...
      if (activeBreakdown && activeBreakdown.parentRowId === rowId) {
  ...
  // Determines available drilldown options based on context
   const getTooltipOptions = (contextType, contextData) => {
    ...
    const hierarchy = get(rowDimension, 'hierarchy', []); // e.g., ["Quarter", "Month", "Week"]
    const parentHierarchyLevel = hierarchy[level] || `Level ${level}`;
    const childHierarchyLevel = hierarchy[level + 1] || `Level ${level + 1}`;

    switch (contextType) {
      case 'rowLevel0Collapsed': // Clicked on the combined cell (Quarter level, columns collapsed)
        if (hasChildren) {
        ...
        break;
      case 'rowLevel0ExpandedCell': // Hovered on a specific cell (Quarter level, columns expanded)
      case 'rowLevel1ExpandedCell': // Hovered on a specific cell (Month level, columns expanded)
         if (hasChildren) {
         ...
         break;
      case 'rowLevel0Label': // Clicked/Hovered on the Quarter label
      case 'rowLevel1Label': // Clicked/Hovered on the Month label
         if (hasChildren) {
    ...
    switch (option.action) {
      case 'expandRow': // Expand row AND columns if columns were collapsed
       ...
       case 'toggleRowOnly': // Just expand/collapse the row
          if (rowId) {
             ...
             if (expandedRows.has(rowId) && activeBreakdown && activeBreakdown.parentRowId === rowId) {
                 setActiveBreakdown(null);
             }
          }
          break;
      ...
      case 'setActiveBreakdown': // Show child rows for a specific column (now applies to Quarter or Month)
        if (rowId && columnId && rowData && rowData.children && rowData.children.length > 0) {
          ...
          setActiveBreakdown({ parentRowId: rowId, columnId: columnId });
        }
        break;
      case 'viewAllColumns': // Clear specific column breakdown, ensure columns expanded
         ...
         setActiveBreakdown(null); // Clear breakdown filter
  ...
  const renderRow = (item, level = 0) => {
    ...
    const isUnderActiveBreakdown = activeBreakdown && parentId === activeBreakdown.parentRowId;
    // activeColumnId now refers to the lowest level (country), breakdown logic might need rethinking
    const activeColumnId = isUnderActiveBreakdown ? activeBreakdown.columnId : null; 

    // Calculate percentage using the aggregated value for the row
    let percentageOfGrandTotal = null;
    if (grandTotal > 0) {
        const rowTotal = calculateRowAggregatedValue(item); // Get total aggregated value for the row
        ...
        >
          ...
          {visibleColumns.map(col => {
            ...
            if (col.isCombined) {
                ...
                cellValue = calculateRowAggregatedValue(item);
            ...
            } else if (col.isCollapsedRegion) {
                 // Use aggregated value for the specific collapsed region
                 cellValue = calculateRowAggregatedValue(item, col.regionId);
                 cellStyle.color = level > 0 ? (level === 1 ? '#cbd5e0' : '#a0aec0') : '#e2e8f0';
                 cellStyle.backgroundColor = 'rgba(0,0,0,0.1)';

            } else if (col.isCountry) {
                // Use aggregated value for the specific country
                cellValue = calculateRowAggregatedValue(item, col.regionId, col.id);
          ...
          {!allColumnsCollapsed && mainColumnDimension && (
            <tr>
              {regionColumns.map(region => {
                 const isExpanded = expandedRegions.has(region.id);
                 const regionCountries = region.columns || [];
                 const colSpan = isExpanded ? regionCountries.length : 1;
                 const width = isExpanded 
                     ? regionCountries.reduce((sum, country) => sum + (country.width || 90), 0) 
                     : (region.collapsedWidth || 100);
                 ...
                 return (
                    <th 
                       ...
                       style={{
                           ...
                           minWidth: `${width}px`, 
                           width: `${width}px`,
                           cursor: region.canCollapse ? 'pointer' : 'default',
                           backgroundColor: '#1a1a1a',
                           borderLeft: '1px solid #333', 
                           borderBottom: '1px solid #333',
                           position: 'relative',
                           paddingLeft: region.canCollapse ? '24px' : '4px' // Space for icon
                       }}
                       onClick={() => region.canCollapse && toggleRegionExpansion(region.id)} // Toggle this region
                    >
                       {region.canCollapse && (
          ...
          {!allColumnsCollapsed && mainColumnDimension && (
            <tr>
              {visibleColumns.map(col => (
                 ...
                 >
                    {/* Only show title for actual countries, collapsed region title is in row above */}
                    {col.isCountry ? col.title : ''} 
                 </th>
              ))}
            </tr>
          )}
            ...
            {visibleColumns.map(col => {
               ...
               if (col.isCombined) {
                  totalValue = grandTotal;
                   ...
                   // Sum totals for the collapsed region
                   let regionTotal = 0;
                   const currentRegion = regionColumns.find(r => r.id === col.regionId);
                   if (currentRegion && currentRegion.columns) {
                       currentRegion.columns.forEach(country => {
                           regionTotal += get(columnTotals, country.id, 0);
                       });
                   }
